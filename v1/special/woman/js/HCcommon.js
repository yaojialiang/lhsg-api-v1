...
HTMLBodyElement.prototype.C = HTMLBodyElement.prototype.getElementsByClassName;
HTMLDivElement.prototype.C = HTMLDivElement.prototype.getElementsByClassName;
HTMLSpanElement.prototype.C = HTMLSpanElement.prototype.getElementsByClassName;
HTMLImageElement.prototype.C = HTMLImageElement.prototype.getElementsByClassName;
...
HCEPG.ButtonManager = HCEPG.BM = {
    ...
    init:function(opt){
        ...
        // 历史焦点
        this.historyFocus = this.historyFocus || {}; 
    },
    // 移动动作
    moveAction:function(key){
        ...
        // 针对整个按钮组
        if(!des &&btnClsRules[dir]){
            des = btnClsRules[dir]
        }

        // 目标值是函数，执行该函数返回新目标值
        if(des instanceof Function){
            des = des(key,_this.btnDom,_this.btnCls,_this.btnIdx);
    ...
    // 滑动
    scroll:function(view,scroll,nextBtn){
        if(view && scroll && nextBtn){
            // 水平滑动
            if( nextBtn.offsetLeft + nextBtn.offsetWidth > scroll.offsetLeft*-1 + view.offsetWidth ){
                scroll.style.left = scroll.offsetLeft - ((nextBtn.offsetLeft + nextBtn.offsetWidth) - (scroll.offsetLeft*-1 + view.offsetWidth)) + 'px';
            }
            else if( nextBtn.offsetLeft < scroll.offsetLeft*-1 ){
                scroll.style.left = scroll.offsetLeft + (scroll.offsetLeft*-1 - nextBtn.offsetLeft) + "px";
            }
            // 垂直滑动
            if( nextBtn.offsetTop + nextBtn.offsetHeight > scroll.offsetTop*-1 + view.offsetHeight ){
                scroll.style.top = scroll.offsetTop - ((nextBtn.offsetTop + nextBtn.offsetHeight) - (scroll.offsetTop*-1 + view.offsetHeight)) + 'px';
            }
            else if( nextBtn.offsetTop < scroll.offsetTop*-1 ){
                scroll.style.top = scroll.offsetTop + (scroll.offsetTop*-1 - nextBtn.offsetTop) + "px";
            }
        }
    },

    // // 垂直滑动
    // function y_scroll(view,scroll,nextBtn){
    //     if(view && scroll && nextBtn){
    //         if( nextBtn.offsetTop + nextBtn.offsetHeight > scroll.offsetTop*-1 + view.offsetHeight ){
    //             scroll.style.top = scroll.offsetTop - ((nextBtn.offsetTop + nextBtn.offsetHeight) - (scroll.offsetTop*-1 + view.offsetHeight)) + 'px';
    //         }
    ...
    rect:function(opt){
        ...
        data.forEach(function(obj,idx){
            ...
            if(idx%cols == cols - 1 && unit.offsetLeft + unit.offsetWidth > boxWidth){
                boxWidth = unit.offsetLeft + unit.offsetWidth + getStyle(unit,'margin-right').match(/\d*/)[0]*1 + 1
                temp.style.width = boxWidth+'px';
            }
            if(Math.ceil((idx+1)/cols) == rows && unit.offsetTop + unit.offsetHeight > boxHeight){
                boxHeight = unit.offsetTop + unit.offsetHeight + getStyle(unit,'margin-bottom').match(/\d*/)[0]*1 + 1
